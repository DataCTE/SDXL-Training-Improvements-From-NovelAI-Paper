...
def get_sigmas(num_inference_steps=28, sigma_min=0.0292, sigma_max=20000.0):
    """Generate sigmas using a cosine schedule that supports Zero Terminal SNR"""
    # Use log space for better numerical stability with high sigmas
...
# Initialize frozen VGG model for perceptual loss
def get_frozen_vgg():
...
def training_loss(model, x_0, sigma, text_embeddings, text_embeddings_2, pooled_text_embeds_2, timestep, target_size):
    ...
    try:
        ...
        # Get initial decode with gradient scaling for stability
        with torch.cuda.amp.autocast():
            initial_output = model.initial_decode(x_0)
            
            # Scale perceptual loss based on sigma to prevent instability
            perceptual_weight = torch.clamp(1.0 / (1.0 + sigma), max=0.1)
            perceptual_loss = perceptual_weight * F.mse_loss(
                frozen_vgg(initial_output),
                frozen_vgg(x_0)
            )
            
            # Add MSE loss with similar scaling
            mse_loss = perceptual_weight * F.mse_loss(initial_output, x_0)

            # Get model prediction with gradient scaling
            model_output = model(
                x_t,
                sigma,
            ...
            # Calculate diffusion loss with improved SNR weighting
            snr = compute_snr(sigma)
            min_snr_gamma = torch.clamp(snr, max=5.0)
            diffusion_loss = min_snr_gamma.mean() * F.mse_loss(
                model_output, noise, reduction='none'
            ).mean([1, 2, 3]).mean()

            # Combine losses with dynamic weighting
            loss = diffusion_loss + perceptual_loss + mse_loss
